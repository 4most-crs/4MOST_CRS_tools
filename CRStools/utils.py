...
def plot_moll(hmap, whmap=None, min=None, max=None, nest=False, title='', label=r'[$\#$ deg$^{-2}$]', filename=None, show=True, mask_dir=None, euclid_fp=False, stardens=False, lsst_fp=False,
    ...
    if fourmost_footprint:
        ...
        for reg in ['ngc','sgc']:
            ...
            if reg == 'ngc':
                ...
                ax.plot(ttt, projection_dec([-20]*100)[mm], lw=0.8, c='orangered', ls='--', zorder=100, label='BG cut')
            pol[0] -= np.radians(115)
            pol[0] += np.radians(rot)
            pol[0] = np.remainder(pol[0] + np.pi*2, np.pi*2)
            pol[0][pol[0] > np.pi] -= np.pi*2
            ax.plot(pol[0], pol[1], color="orangered", lw=2, zorder=100)
        ax.plot(pol[0], pol[1], color="orangered", lw=2, zorder=100, label='4MOST')
    ...
    if lsst_fp:
        ...
        plt.fill_between(np.radians(ra[ra.argsort()]),np.radians(dec[ra.argsort()]), interpolate=True, color='lightskyblue', alpha=0.2, label='LSST')
